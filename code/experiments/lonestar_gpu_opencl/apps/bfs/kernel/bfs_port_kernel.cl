...
void drelax(__global foru *dist,
  ...
  if (iteration == 0) {
    ...
    if (processnode2(dist, graph, inwl, outwl, gather_offsets, queue_index, scan_arr, loc_tmp, iteration, gl_ctx, local_ctx)) {
      *gerrno = 1;
    }
  }
}
...
__kernel void drelax2(__global foru *dist,
  ...
  if (iteration == 0) {
    drelax(dist, graph, gerrno, inwl, outwl, gather_offsets, &queue_index, scan_arr, &loc_tmp, iteration, gl_ctx, &local_ctx);
  }
  else {
    ...
    while (*(in->dindex) > 0) {
      ...
      drelax(dist, graph, gerrno, in, out, gather_offsets, &queue_index, scan_arr, &loc_tmp, iteration, gl_ctx, &local_ctx);
