...
const char * CL_FILE = STRINGIFY(KERNEL_DIR) "mst_non_port_kernel.cl";
...
int main(int argc, char *argv[]) {
  ...
  int awgs = atoi(argv[2]);
  int awgn = atoi(argv[3]);
  ...
  // Create and initialise non-portable barrier
  cl_mem d_gbar_arr;
  err = allocate_and_init_gbar(&context, &queue, &d_gbar_arr,  awgn);
  if (err < 0 ) { perror("failed allocating gbar_array"); exit(1); }
  ...
  err |= clSetKernelArg(dfindcompmintwo, 11, sizeof(void *), (void *) &d_gbar_arr);
  if (err < 0 ) { printf("failed setting kernel arg dfindcompmintwo %d\n", err); exit(1); }

  size_t local_work_active[3]  = { awgs,  1, 1 };
  size_t global_work_active[3] = { awgn * awgs, 1, 1 };
  ...
  do {
    ...
    err = clEnqueueNDRangeKernel(queue,
                                 ...
                                 0,
                                 0);
    if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: dfindelemins (%d)\n", err); return -1; }
  ...
  // Print timing and solution information
