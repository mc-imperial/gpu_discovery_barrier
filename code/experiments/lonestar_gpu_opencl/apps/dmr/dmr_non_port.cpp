...
int WGN;  // Total number of threads
int awgn; // Number of workgroups

const char * CL_FILE = STRINGIFY(KERNEL_DIR) "dmr_non_port_kernel.cl";
...
void refine_mesh(ShMesh &mesh) {
  ...
  // Create and intialise unsafe global barrier
  ...
  err |= clSetKernelArg(refine, 5, sizeof(void *), (void*) &d_gbar_arr);
  CHECK_ERR(err);
...
int main(int argc, char *argv[]) {
  ...
  if(argc != 5) {
      printf("Usage: %s basefile <workgroup size> <workgroup number> <maxfactor>\n", argv[0]);
      exit(0);
    }

  maxfactor = atoi(argv[4]);
  WGS = atoi(argv[2]);
  WGN = WGS*atoi(argv[3]);
  awgn = atoi(argv[3]);
